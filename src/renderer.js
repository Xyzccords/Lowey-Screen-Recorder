...
// suena cuando se dispara un ATAJO de teclado (no al clickear el botón).
// Si la próxima vez el atajo "no hace nada" pero este beep sí se escucha,
// el problema está en la lógica de la app (algo arreglable); si el beep
// nunca suena, la tecla no le está llegando a la app (otro programa se la
// está comiendo antes de que la vea Electron) — eso ya no se puede arreglar
// desde acá, pero al menos se sabe con certeza cuál de los dos es.
...
async function stopRecording() {
  ...
  // "Grabaciones sin optimizar" y el botón se libera al toque. La duración
  // real descuenta el tiempo que estuvo pausada (el video final tampoco
  // incluye esos tramos, se cortan del archivo).
...
async function togglePause() {
  ...
  if (!isPaused) {
    ...
    window.lowey.notifyRecordingPaused();
    await window.lowey.pauseVideoCapture(videoCaptureId);
  } else {
    ...
    window.lowey.notifyRecordingResumed(recordStart + accumulatedPauseMs);
    await window.lowey.resumeVideoCapture(videoCaptureId);
  }
}
...
window.lowey.onToggleRecordingShortcut(() => {
  playShortcutBeep();
  toggleRecording();
});
window.lowey.onTogglePauseShortcut(() => {
  playShortcutBeep();
  togglePause();
});
...
async function loadShortcutHint() {
  const recordShortcut = await window.lowey.getRecordShortcut();
  const pauseShortcut = await window.lowey.getPauseShortcut();
